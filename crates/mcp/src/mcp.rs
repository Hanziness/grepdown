...
pub struct DocIdParams {
    ...
    #[schemars(
        description = "Unique document identifier (typically the file path relative to the project root)"
    )]
    pub doc_id: String,
}
...
pub struct ApproveEditsParams {
    ...
    #[schemars(
        description = "Only approve stale references in these paths. Empty or omitted means approve all."
    )]
...
pub struct ReachableParams {
    ...
    #[schemars(
        description = "Starting document identifier (typically the file path relative to the project root)"
    )]
    pub doc_id: String,
...
impl GrepdownMCP {
    ...
    pub async fn refresh(&self) -> Result<String, String> {
        let project = self.project.lock().await;
        let result = project.refresh().map_err(|e| e.to_string())?;
    ...
    pub async fn lint(&self) -> Result<String, String> {
        ...
        let result = run_lints(conn).map_err(|e| e.to_string())?;
    ...
    #[tool(
        ...
        annotations(
            title = "Approve stale references",
            read_only_hint = false,
    ...
    pub async fn approve_edits(
        &self,
        Parameters(ApproveEditsParams { paths }): Parameters<ApproveEditsParams>,
    ) -> Result<String, String> {
        let project = self.project.lock().await;
        let conn = project.get_conn();
        let count = approve_edits(conn, &paths.unwrap_or_default()).map_err(|e| e.to_string())?;

        serde_json::to_string(&count).map_err(|e| format!("serialization error: {e}"))
    }
    ...
    pub async fn get_links(
        &self,
        Parameters(DocIdParams { doc_id }): Parameters<DocIdParams>,
    ) -> Result<String, String> {
        let project = self.project.lock().await;
        let outgoing = project.get_links_from(&doc_id).map_err(|e| e.to_string())?;
        let incoming = project.get_links_to(&doc_id).map_err(|e| e.to_string())?;
    ...
    pub async fn get_reachable(
        &self,
        Parameters(ReachableParams { doc_id, max_depth }): Parameters<ReachableParams>,
    ) -> Result<String, String> {
        let project = self.project.lock().await;
        let result: Vec<ReachableNode> = project
            .get_reachable(&doc_id, max_depth.unwrap_or(2))
            .map_err(|e| e.to_string())?;
    ...
    pub async fn get_document(
        ...
        let project = self.project.lock().await;
        let result: DocumentContent = project.read_document(&doc_id).map_err(|e| e.to_string())?;
